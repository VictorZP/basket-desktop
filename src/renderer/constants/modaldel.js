export const MODAL_DEL = {
	TITLE: "Удалить?",
	CONFIRM_TEXT: "Подтверждаете удаление ",
	CHAMP_TEXT:
		"Обратите внимание, что также будут удалены все связаные с этим чемпионатом команды!",
	YES: "Да",
	NO: "Нет",
	PAGE_TYPE_C: "delCyber",
	PAGE_TYPE_CHAMP: "delChampionship",
	PAGE_TYPE_TEAM_NAME: "delTeamName",
	PAGE_TYPE_MATCHES: "delMatches",
	PAGE_TYPE_PARCER_URL: "delParcerUrl",
	PAGE_TYPE_LINES_DATA: "delLinesData",
	PAGE_TYPE_PARCER_DATA: "delParcerData",
	PAGE_TYPE_FILTER_CHAMP: "delParcerChamp",
	PAGE_TYPE_RESULTS_BY_SEASON: "delMatchesResultsBySeason",
	PAGE_TYPE_HALVES_STATISTICS_DATA: "delHalvesStatisticsData",
};
